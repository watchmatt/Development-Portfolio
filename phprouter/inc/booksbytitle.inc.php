...
<?php
function getbooksbytitle(string $titlesearch)
{
	...
	$query1->bind_param('s',$titlesearch);
...
# Table loop with results
	$results = '<table><tr><th>Title</th><th>Year</th><th>Publisher</th></tr>';
	while($query1->fetch()){
		$results .= '<tr><td><a href="bookinfo/' . $book_id . '">' . $title . '</a></td>';
		$results .= "<td>$pubyear</td>";
		$results .= "<td>$pubname</td></tr>";
